...
st.title("🕵️ Unified Job Market Intelligence")
...
def render_resource_card(item, type_label):
    ...
    # Generic fallback image if URL is missing or broken
...
if scrape_btn:
    if not (use_linkedin or use_indeed or use_naukri):
        ...
        if df.empty:
            ...
            # === TAB 2: RAW DATA (Isolated) ===
            with tab2:
                st.subheader("📋 Scraped Job Listings")
                st.dataframe(df, use_container_width=True, height=600)

            # === TAB 3: LEARNING PATH (Isolated) ===
            with tab3:
